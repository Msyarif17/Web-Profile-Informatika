@if ($page->deleted_at)
    <form action="{{ route('dash.page.restore', $page->id) }}" method="page" class="d-inline">
        @method('PATCH')
        @csrf
    ...
    <form action="{{ route('dash.page.forceDelete', $page->id) }}" method="page" class="d-inline"
...
@else
    <a href="{{ route('dash.page.edit', $page->id) }}" class="btn  btn-primary btn-flat btn-sm" data-toggle="tooltip"
        data-placement="top" title="edit"><span class="fa fa-edit"></span></a>
    
    <form action="{{ route('dash.page.destroy', $page->id) }}" method="page" class="d-inline"
        onsubmit="return confirm('apakah anda yakin?')">
        @csrf
        <input type="hidden" name="_method" value="delete" />
