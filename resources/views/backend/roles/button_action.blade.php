@if ($roles->deleted_at)
    <form action="{{ route('dash.roles.restore', $roles->id) }}" method="post" class="d-inline">
        @method('PATCH')
        @csrf
    ...
    <form action="{{ route('dash.roles.forceDelete', $roles->id) }}" method="post" class="d-inline"
...
@else
    <a href="{{ route('dash.roles.edit', $roles->id) }}" class="btn  btn-primary btn-flat btn-sm" data-toggle="tooltip"
        data-placement="top" title="edit"><span class="fa fa-edit"></span></a>
    
    <form action="{{ route('dash.roles.destroy', $roles->id) }}" method="post" class="d-inline"
        onsubmit="return confirm('apakah anda yakin?')">
        @csrf
        <input type="hidden" name="_method" value="delete" />
