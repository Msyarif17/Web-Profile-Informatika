@if ($footer->deleted_at)
    <form action="{{ route('dash.footer.restore', $footer->id) }}" method="post" class="d-inline">
        @method('PATCH')
        @csrf
    ...
    <form action="{{ route('dash.footer.forceDelete', $footer->id) }}" method="post" class="d-inline"
...
@else
    <a href="{{ route('dash.footer.edit', $footer->id) }}" class="btn  btn-primary btn-flat btn-sm"
    ...
    <form action="{{ route('dash.footer.destroy', $footer->id) }}" method="post" class="d-inline"
        onsubmit="return confirm('apakah anda yakin?')">
        @csrf
        <input type="hidden" name="_method" value="delete" />
